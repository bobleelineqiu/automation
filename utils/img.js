    ...
    "Mozilla/4.0 (compatible; MSIE 6.0; ) Opera/UCWEB7.0.2.37/28/999"
     ...
     let options={
            ...
            headers: {
                ...
                'Referer': config.searchPath,
                "Upgrade-Insecure-Requests": 1,
                "User-Agent": header_pc[Math.floor(Math.random()*20)]
            }
        };
        
        await request(options).pipe(fs.createWriteStream(file))
};
...
exports.convert2Img = async (src, dir) => {
  if (/^http/.test(src)) {
    ...
    // await base642Img(urlToBase64(src), dir);
  } else {
